...
case "${ACTION}" in
    ...
    apply)
        ...
        if [ ! -f "${STORAGE_SH}" ]; then
            log "ERROR: Script not found: ${STORAGE_SH}"
            json_response false "Script not found: ${STORAGE_SH}"
            exit 0
        fi
        
        if [ ! -x "${STORAGE_SH}" ]; then
            log "ERROR: Script not executable: ${STORAGE_SH}"
            json_response false "Script not executable: ${STORAGE_SH}"
            exit 0
        fi
        
        log "Executing: ${STORAGE_SH} ${HDD_BAY} ${SSD_BAY}"
        RESULT=$("${STORAGE_SH}" "${HDD_BAY}" "${SSD_BAY}" 2>&1)
        EXIT_CODE=$?
        ...
        if [ ${EXIT_CODE} -eq 0 ]; then
            ...
            json_response false "Apply failed: ${RESULT}"
        fi
        ;;
    ...
    restore)
        ...
        log "Executing: ${STORAGE_SH} -r"
        RESULT=$("${STORAGE_SH}" -r 2>&1)
        EXIT_CODE=$?
        ...
        if [ ${EXIT_CODE} -eq 0 ]; then
            ...
            json_response false "Restore failed: ${RESULT}"
        fi
        ;;
